...
class PdfCompliancePolicyWorker
  ...
  def perform(policy_id, single_cpolicy)
    cpolicy = CompliancePolicy.find(policy_id)
    if single_cpolicy
      update_single(cpolicy, nil, nil, nil)
    else
  ...
  def update_single(published_cpolicy, current_page, toc, combined_policies)
    ...
    loaded_pdf.number_pages(location: [:bottom], font_size: 8, margin_from_side: 0, margin_from_height: 20, start_at: current_page) unless current_page.nil?
    loaded_pdf.save file.path
    uploader = PdfUploader.new(:store)
    uploaded_file = uploader.upload(file)
    page_count = loaded_pdf.pages.count
    combined_policies << loaded_pdf unless combined_policies.nil?
    ...
    file.delete
    [current_page, toc, combined_policies]
  end
end
