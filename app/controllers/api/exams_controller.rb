...
class Api::ExamsController < ApiController
  ...
  def email
    if @auditor.present?
      ...
      respond_with error: 'Access denied', status: :unprocessable_entity
    end
  end

  def show
    return respond_with error: 'Access denied', status: :unprocessable_entity if @auditor.nil?
    ...
    if @auditor.retries < 4 && @auditor.otp == params[:otp]
      if @auditor.otp_requested > Time.zone.now - 5.minutes
        ...
        respond_with error: 'Timeout', status: :unprocessable_entity
      end
    else
      respond_with error: 'Access denied', status: :unprocessable_entity
    end
