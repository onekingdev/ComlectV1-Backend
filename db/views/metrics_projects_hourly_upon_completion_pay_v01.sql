SELECT
  'projects_hourly_upon_completion_pay'::varchar AS metric,
  mtd.cnt AS mtd,
  fytd.cnt AS fytd,
  itd.cnt AS itd
FROM
(SELECT COUNT(*) AS cnt
 FROM projects
 WHERE type = 'one_off' AND pricing_type = 'hourly' AND payment_schedule = 'upon_completion' AND
...
(SELECT COUNT(*) AS cnt
 FROM projects
 WHERE type = 'one_off' AND pricing_type = 'hourly' AND payment_schedule = 'upon_completion'
) AS itd
