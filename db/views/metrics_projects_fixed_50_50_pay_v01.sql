SELECT
  'projects_fixed_50_50_pay'::varchar AS metric,
  mtd.cnt AS mtd,
  fytd.cnt AS fytd,
  itd.cnt AS itd
FROM
(SELECT COUNT(*) AS cnt
 FROM projects
 WHERE type = 'one_off' AND pricing_type = 'fixed' AND payment_schedule = 'fifty_fifty' AND
...
(SELECT COUNT(*) AS cnt
 FROM projects
 WHERE type = 'one_off' AND pricing_type = 'fixed' AND payment_schedule = 'fifty_fifty'
) AS itd
