...
UNION SELECT 'postings_job_share'::varchar AS metric,
  (j.cnt::float / a.cnt) * 100 AS value
FROM
  (SELECT NULLIF(COUNT(*), 0) AS cnt FROM all_projects) AS a,
  (SELECT NULLIF(COUNT(*), 0) AS cnt FROM job_revenue) AS j

UNION SELECT 'postings_project_share'::varchar AS metric,
  (p.cnt::float / a.cnt) * 100 AS value
FROM
  (SELECT NULLIF(COUNT(*), 0) AS cnt FROM all_projects) AS a,
  (SELECT NULLIF(COUNT(*), 0) AS cnt FROM project_revenue) AS p
